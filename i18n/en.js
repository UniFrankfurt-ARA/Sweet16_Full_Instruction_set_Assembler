...
(function(global) {
  global.I18N_EN = {
    ...
    msg: {
      ...
      programFinished: "Program finished ({reason}). Memory preserved — first press of Run Next rewinds PC to the start only (no fetch); second press runs again.",
      programFinishedAfterRun: "Run All finished. PC is past the end — inspect memory. Run Next: rewind to top without executing; then run again as usual.",
      rewindPcNoExecute: "PC rewound to start — no instruction executed (memory unchanged). Next step runs the first instruction.",
    ...
    instrDesc: {
      ...
      STO: "Store to user memory: STO [Rs], Rt → Mem[Rs] = Rt.",
      LDD: "Load from user memory: LDD Rd, [Rs] → Rd = Mem[Rs].",
