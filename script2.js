...
function loadSamplePrograms() {
  ...
  tabContent.innerHTML = data.map((program, index) => `
    <div class="sample-program-block">
      ...
      <button type="button" class="add-program-btn use-instr-btn" data-program-index="${index}">Add</button>
      <hr>
    </div>
  `).join("");
}
...
function loadInstructionSet() {
  ...
  tabContent.innerHTML = `<div class="instruction-set-content">${sorted.map(instruction => `
    <div class="instr-block">
      ...
      ${instruction.useLine != null ? `<button type="button" class="use-instr-btn" data-use-line="${escapeAttr(instruction.useLine)}">Use</button>` : ""}
    </div>
    <hr>
  `).join("")}</div>`;
}
...
function loadAliases() {
  ...
  tabContent.innerHTML = Object.entries(groups).map(([cat, aliases]) => `
    <div class="alias-group">
      <h2 class="alias-group-title">${cat}</h2>
      ${aliases.map(a => `
        <div class="alias-block">
          ...
          <p class="alias-desc">${escCode(a.description)}</p>
          <div class="alias-footer">
            <span class="alias-use-label">Usage:</span>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tabContent      = $("tabContent");
  const allBottomTabs   = [tabPrograms, tabInstructions, tabAliases, tabManual];
  ...
  importMemoryFile?.addEventListener("change", (e) => {
    ...
    reader.onload = () => {
      ...
      if (typeof window.importDynamicMemoryFromText === "function") {
        ...
        alert("Imported " + n + " memory location(s).");
      }
    };
    reader.readAsText(file);
    e.target.value = "";
  ...
  function setBottomTab(active) {
    allBottomTabs.forEach(t => t?.classList.remove("active"));
    active?.classList.add("active");
  }

  tabPrograms?.addEventListener("click", () => {
    setLeftColumnMode("info");
    setBottomTab(tabPrograms);
    loadSamplePrograms();
  });

  tabInstructions?.addEventListener("click", () => {
    setLeftColumnMode("info");
    setBottomTab(tabInstructions);
    loadInstructionSet();
  });

  tabAliases?.addEventListener("click", () => {
    setLeftColumnMode("info");
    setBottomTab(tabAliases);
    loadAliases();
  });

  tabManual?.addEventListener("click", () => {
    setLeftColumnMode("info");
    setBottomTab(tabManual);
    ...
    const formattedManual = manual
      ...
      .replace(/# (.*?)\n/g, "<h1>$1</h1>\n")
      .replace(/\n/g, "<br>");
    if (tabContent) tabContent.innerHTML = formattedManual
      ? `<div class="user-manual-content">${formattedManual}</div>`
      : "<p>Manual not loaded.</p>";
  });
  ...
  tabInMemoryData?.addEventListener("click", () => {
    tabInMemoryData.classList.add("active");
    tabMachineCode?.classList.remove("active");
    window.memoryViewTab = "data";
    if (machineCodeActions) machineCodeActions.style.display = "none";
    if (typeof updateMemoryDisplay === "function") updateMemoryDisplay();
    else if (inMemoryProgramEl) inMemoryProgramEl.textContent = window.currentProgramDisplayText || "";
  });
  tabMachineCode?.addEventListener("click", () => {
    tabMachineCode.classList.add("active");
    tabInMemoryData?.classList.remove("active");
    ...
    if (inMemoryProgramEl) inMemoryProgramEl.textContent = hexText || "(no program in memory)";
  });
  ...
  $("copyMachineCode")?.addEventListener("click", () => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      window.showToast?.("Machine code copied to clipboard.");
    }).catch(() => {
      window.showToast?.("Copy failed — please select and copy manually.");
    });
  });
